...
DO $$ BEGIN
  CREATE EXTENSION IF NOT EXISTS supabase_vault CASCADE;  -- vault.secrets / vault.decrypted_secrets
EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'supabase_vault: %', SQLERRM; END $$;
-- CASCADE тянет pgsodium (уже создан выше — IF NOT EXISTS, безвредно).
